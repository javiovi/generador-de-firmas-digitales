...
// Definir la interfaz para la firma
interface Signature {
  primaryColor: string
  logoUrl: string
  photoUrl: string
...
// Función auxiliar para generar la URL del icono
function getIconUrl(type: string, color: string): string {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  return `${baseUrl}/api/icon?type=${type}&color=${encodeURIComponent(color)}`
}
...
function generateHtmlCode(
  ...
  exportOnlyTable: boolean = true // Por defecto true
): string {
...
export function generateHtmlCodeWithPngIcons(
  ...
  // Asegurar que todas las tablas tengan los atributos necesarios
  html = html.replace(
    /<table([^>]*)>/g,
    (match, attributes) => {
      if (!attributes.includes('cellpadding=')) attributes += ' cellpadding="0"'
      if (!attributes.includes('cellspacing=')) attributes += ' cellspacing="0"'
      if (!attributes.includes('border=')) attributes += ' border="0"'
      return `<table${attributes}>`
    }
  )

  // Asegurar que todos los enlaces tengan los atributos necesarios
  html = html.replace(
    /<a([^>]*)>/g,
    (match, attributes) => {
...
export default function SignatureGenerator() {
  ...
  const [signatureData, setSignatureData] = useState<Signature>({
    ...
    photoSize: 100,
    socialLinks: {
      facebook: { platform: 'facebook', url: '', enabled: false },
      instagram: { platform: 'instagram', url: '', enabled: false },
      linkedin: { platform: 'linkedin', url: '', enabled: false },
      twitter: { platform: 'twitter', url: '', enabled: false },
      youtube: { platform: 'youtube', url: '', enabled: false }
    }
  })
  ...
  const handleSocialToggle = (network: keyof Signature['socialLinks']) => {
    setSignatureData((prev) => ({
      ...prev,
      socialLinks: {
        ...prev.socialLinks,
